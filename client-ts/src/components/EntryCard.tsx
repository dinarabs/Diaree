...
const EntryCard: FC<EntryCardProps> = ({ diaryEntry, onDelete }) => {
  ...
  const truncatedText = diaryEntry.text.length > 25
  ...
  return (
    <div className="flex items-start justify-between shadow-md w-full mx-0 my-4 h-32 p-4 border-2 border-light-grey rounded-lg">
      ...
      <div className="flex flex-col items-end">
        <UilTimes size={32} onClick={handleDeleteClick} style={{ cursor: 'pointer' }} />
        <h5 className="text-sm pt-4 text-grey">{moment.utc(diaryEntry.createdAt).format('MMMM Do, YYYY - h:mm a ')}</h5>
      </div>
    </div>
  )
}
